...
Select p.Proposal_ID
  ...
  From Proposal                               p
     ...
     --Add below sub-query to select the Max Questionnarire_Answer_Headr_ID when
     --when there are two or more completed Answers in the Questionnaire_Answer table, 03/11/2013.
     , (Select substr(qh.Module_Item_Key, 1, instr(qh.Module_Item_Key, '|', 1)-1) as pd_Prop_No
             , qh.Questionnaire_Completed_Flag
             , max(qh.Questionnaire_Answer_Header_ID) as Max_AH_ID
          From Questionnaire_Answer_Header            qh
        Group by substr(qh.Module_Item_Key, 1, instr(qh.Module_Item_Key, '|', 1)-1)
                        , qh.Questionnaire_Completed_Flag ) mqh
     ...
     , (Select p.Proposal_ID
          ...
          Where p.sequence_number = 1
            ...
            AND qah.Questionnaire_Answer_Header_ID = mqh.Max_AH_ID
            and qah.Questionnaire_Ref_ID_FK in (1000, 38665, 49435, 67348)
     ...
     , (Select p.Proposal_ID
          ...
          From Proposal                                    p
              ...
              , eps_Prop_Person                            pp
              , Questionnaire_Answer_Header                qah 
              --Don't need to add the sub-query here because question 49429 is 
              --not in answers of latest version.
              /*, (Select substr(qh.Module_Item_Key, 1, instr(qh.Module_Item_Key, '|', 1)-1) as pd_Prop_No
                      , qh.Questionnaire_Completed_Flag
                      , max(qh.Questionnaire_Answer_Header_ID) as Max_AH_ID
                   From Questionnaire_Answer_Header            qh
          ...
          Where p.sequence_number = 1
            ...
            and qah.Questionnaire_Answer_Header_ID = qa.Questionnaire_AH_ID_FK
            and qah.Questionnaire_Ref_ID_FK in (1000, 38665, 49435, 67348)
            --AND qah.Questionnaire_Answer_Header_ID = mqh.Max_AH_ID --not in the latest version 
            --AND qah.Questionnaire_Completed_Flag = 'Y'             --not in the latest version
            and qq.questionnaire_Ref_ID_FK = qah.Questionnaire_Ref_ID_FK
            AND qq.Question_Number = qa.Question_Number
 ...
 Where p.sequence_number = 1
   ...
   and qah.Questionnaire_Answer_Header_ID = qa.Questionnaire_AH_ID_FK
   AND qah.Questionnaire_Answer_Header_ID = mqh.Max_AH_ID
   --A new Questionnaire_Ref_ID 67348 added in Questionnaire table because Andrew changed
   -- the name from " UH Proposal Person Certification" to "Key Personnel Certifications" on 12/12/2012.
   and qah.Questionnaire_Ref_ID_FK in (1000, 38665, 49435, 67348) 
   AND qah.Questionnaire_Completed_Flag = 'Y'
   and qq.questionnaire_Ref_ID_FK = qah.Questionnaire_Ref_ID_FK
   AND qq.Question_Number = qa.Question_Number
   ...
   --and qa.Question_Ref_ID_FK = 49406 --Change 11082 to 49406 cannot pick up any data tested on 12/10/2012.
